...
class CartRepository {
  ...
  Future<dynamic> addToCart(plantModel data, int quantity) async {
    try {
      ...
      try {
        ...
        if (response.docs.isEmpty) {
          final CartModel _data = CartModel(
              user_id: uid,
              items: [CartItem(product: data, quantity: quantity)]);
          await instance.add(_data);
        } else {
          var cart = response.docs.single.data();
          var checkProduct =
          cart.items.indexWhere((element) => element.product.id == data.id);
          if (checkProduct == -1) {
  ...
  Future<dynamic> removeFromCart(plantModel data) async {
    try {
      if (FirebaseAuth.instance.currentUser == null) {
        return;
      }
  ...
  Future<dynamic> removeItemFromCart(plantModel data) async {
    try {
      if (FirebaseAuth.instance.currentUser == null) {
        return;
      }
